...
class ContrastLoss(nn.Module):
    ...
    def forward(self, model_output, GT_sig, label_flag):
        ...
        samples, _ = self.ST_sampling(model_output)
...
class ST_sampling(nn.Module):
    ...
    def forward(self, input, ext_offsets=None): # input: (2, N, T)
        samples = []
        all_offsets = []
...
class T_sampling(nn.Module):
    ...
    def forward(self, input, ST_block_shape, ext_offsets=None): # input: shape (2, T), ST_block_shape: (2, N, T)
        samples = []
            ...
            for c in range(ST_block_shape[1]):
                for i in range(self.K): # loop for sampling K samples with time length delta_t along temporal dimension
                    if ext_offsets is None:
                        ...
                        offset = ext_offsets[b * ST_block_shape[1] * self.K + c * self.K + i]
